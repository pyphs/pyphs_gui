...
from pyphs.misc.signals.synthesis import names as signals_names
import ast
...
class EditDialog(QDialog):
    ...
    def initUI(self, signal):
        ...
        self.grid.addWidget(QLabel('Signal'), 0, 1)
        self.grid.addWidget(QLabel('Duration'), 0, 2)
        self.grid.addWidget(QLabel('Parameters'), 0, 3)

        self.signal = signal

        self.label = LabelWidget(self.input['label'])

        self.signalQlabel = QLabel('', self)

        self.Qsignal = QComboBox(self)
        self.grid.addWidget(self.label, 1, 0)
        self.grid.addWidget(self.Qsignal, 1, 1)
        self.Qsignal.addItem('Select...')
        ...
        index = self.Qsignal.findText(self.signal['signal'],
                                      Qt.MatchFixedString)
        if index >= 0:
            self.Qsignal.setCurrentIndex(index)

        self.init_comp()
        vbox = QVBoxLayout()
        vbox.addWidget(self.signalQlabel)
    ...
    def onChanged_label(self, text):
        self.input['label'] = str(text)
        self.update_Qlabel()

    def update_Qlabel(self):
        text = print_input(self.input)
